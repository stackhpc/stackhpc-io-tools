...
if [ ! -f $JOB_LOCK ]; then rm -rf $JOB_DIR; touch $JOB_LOCK; fi
export RESULT_DIR=$JOB_DIR/$POD_NAME
mkdir -p $RESULT_DIR
let bs=0; BS_LOCK=${JOB_DIR}/${bs}.lock; mkdir -p $BS_LOCK; touch $BS_LOCK/$POD_NAME
while [ $(ls $BS_LOCK | wc -l) -lt $NUM_PODS ]; do usleep 1000; done
